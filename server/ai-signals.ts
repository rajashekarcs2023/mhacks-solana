// 🧠 AI TRADING SIGNALS GENERATOR - Advanced market analysis and trade recommendations
...
interface MarketContext {
  ...
  newsImpact: 'NEGATIVE' | 'NEUTRAL' | 'POSITIVE';
...
interface SignalPerformance {
  ...
  winRate: number;
  avgReturn: number;
  bestSignal: { return: number; duration: string };
  worstSignal: { return: number; duration: string };
...
export class AITradingSignals {
  ...
  async generateTradingSignal(symbol: string, timeframe: '5M' | '15M' | '1H' | '4H' | '1D' = '1H'): Promise<TradingSignal> {
  ...
  private async generateAISignal(
    ...
    if (aiScore > 25) action = 'BUY';
    else if (aiScore < -25) action = 'SELL';
    else if (Math.abs(aiScore) < 10) action = 'WAIT';
    else action = 'HOLD';
  ...
  private calculateTechnicalIndicators(symbol: string, priceData: any) {
    ...
    return {
      ...
      volume: ['LOW', 'NORMAL', 'HIGH', 'EXTREME'][Math.floor(Math.random() * 4)] as 'LOW' | 'NORMAL' | 'HIGH' | 'EXTREME'
    };
  }

  private generateAIReasoning(symbol: string, action: string, score: number, technicals: any, context: MarketContext): string {
    const reasons = [];
    
    if (action === 'BUY') {
      if (technicals.rsi < 35) reasons.push('oversold conditions (RSI)');
      if (technicals.macd.histogram > 0) reasons.push('bullish MACD crossover');
      if (context.overallSentiment === 'BULLISH') reasons.push('positive market sentiment');
      if (context.liquidity === 'EXCELLENT') reasons.push('strong liquidity support');
    } else if (action === 'SELL') {
      if (technicals.rsi > 65) reasons.push('overbought conditions (RSI)');
      if (technicals.macd.histogram < 0) reasons.push('bearish MACD divergence');
      if (context.overallSentiment === 'BEARISH') reasons.push('negative market sentiment');
    } else if (action === 'WAIT') {
  ...
  private assessRiskLevel(confidence: number, context: MarketContext, technicals: any): 'LOW' | 'MEDIUM' | 'HIGH' {
    let riskScore = 0;
    
    if (confidence < 60) riskScore += 2;
    if (context.volatility === 'HIGH' || context.volatility === 'EXTREME') riskScore += 2;
    if (context.liquidity === 'POOR') riskScore += 2;
    if (technicals.volume === 'LOW') riskScore += 1;
  ...
  private calculatePositionSize(action: string, riskLevel: string, confidence: number): number {
    if (action === 'WAIT' || action === 'HOLD') return 0;
    
    let baseSize = confidence / 100 * 0.1; // Max 10% based on confidence
    
    // Adjust for risk level
    switch (riskLevel) {
      case 'LOW': baseSize *= 1.5; break;
      case 'MEDIUM': baseSize *= 1.0; break;
      case 'HIGH': baseSize *= 0.5; break;
    }
    
    return Math.min(baseSize * 100, 15); // Cap at 15% of portfolio
  }

  private getFundamentalFactors(symbol: string, context: MarketContext): string[] {
    const factors = [
  ...
  private getSeasonalFactors(): string[] {
    ...
    if (month >= 10 || month <= 1) return ['Year-end rally potential', 'Q4 institutional flows'];
    if (month >= 2 && month <= 4) return ['Spring altcoin season', 'Conference season boost'];
    if (month >= 5 && month <= 7) return ['Summer consolidation', 'Lower institutional activity'];
    return ['Back-to-school flows', 'Q3 earnings impact'];
  }

  async getSignalHistory(symbol: string, limit: number = 10): Promise<TradingSignal[]> {
    const history = this.signalHistory.get(symbol) || [];
    return history.slice(-limit);
  }

  async getPerformanceStats(): Promise<SignalPerformance> {
    return this.performance;
  }

  async getTopSignals(timeframe: '24H' | '7D' | '30D' = '24H'): Promise<TradingSignal[]> {
    const cutoffTime = new Date();
    if (timeframe === '24H') cutoffTime.setHours(cutoffTime.getHours() - 24);
    else if (timeframe === '7D') cutoffTime.setDate(cutoffTime.getDate() - 7);
    else cutoffTime.setDate(cutoffTime.getDate() - 30);
  ...
  async generateMarketAlert(): Promise<{
