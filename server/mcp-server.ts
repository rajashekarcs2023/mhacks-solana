...
server.setRequestHandler(CallToolRequestSchema, async (request: CallToolRequest) => {
  try {
    switch (request.params.name) {
      ...
      case "explain_tx": {
        const { signature } = request.params.arguments as { signature: string };
      ...
      case "airdrop_and_tip": {
        const { to, sol = 0.01 } = request.params.arguments as { to: string; sol?: number };
      ...
      case "get_defi_strategies": {
        ...
        const recommendations = await generateStrategyRecommendations(
          connection,
          args.address,
          args.riskTolerance || 'moderate',
          args.investmentAmount || 1000,
          args.timeHorizon || 'medium'
        );
      ...
      case "create_x402_payment": {
        const { to, amount, service, description = "" } = request.params.arguments as {
          to: string;
          amount: number;
          service: string;
          description?: string;
      ...
      case "detect_whale_activity": {
        const { address, threshold = 100000 } = request.params.arguments as { 
          address: string; 
          threshold?: number; 
        };
      ...
      case "generate_trading_signal": {
        const { symbol = "SOL", timeframe = "1H" } = request.params.arguments as { 
          symbol?: string; 
