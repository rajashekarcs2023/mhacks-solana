...
export function enforceGuardrails(params) {
    ...
    if (SAFETY_CONFIG.refuseMainnetByDefault &&
        ...
        throw new Error(`Mainnet operations disabled for safety. Use 'devnet' or set mode: 'i_understand_the_risk'`);
    }
    // Slippage protection
    if (slippageBps !== undefined && slippageBps > SAFETY_CONFIG.maxSlippageBps) {
        throw new Error(`Slippage too high (${slippageBps} bps). Maximum allowed: ${SAFETY_CONFIG.maxSlippageBps} bps (${SAFETY_CONFIG.maxSlippageBps / 100}%)`);
    }
    // Amount limits for tips/transfers
    if (amount !== undefined && amount > SAFETY_CONFIG.maxTipAmount) {
        throw new Error(`Amount too high (${amount} SOL). Maximum allowed: ${SAFETY_CONFIG.maxTipAmount} SOL for demo purposes`);
    }
}
